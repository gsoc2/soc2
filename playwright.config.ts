...
const IS_EMBED_TEST = process.argv.some((a) => a.startsWith("--project=@gsoc2/embed-core"));
const IS_EMBED_REACT_TEST = process.argv.some((a) => a.startsWith("--project=@gsoc2/embed-react"));

const webServer: PlaywrightTestConfig["webServer"] = [
  {
    command:
      "NEXT_PUBLIC_IS_E2E=1 NODE_OPTIONS='--dns-result-order=ipv4first' yarn workspace @gsoc2/web start -p 3000",
    port: 3000,
    timeout: 60_000,
...
if (IS_EMBED_TEST) {
  ...
  webServer.push({
    command: "yarn workspace @gsoc2/embed-core dev",
    port: 3100,
    timeout: 60_000,
    reuseExistingServer: !process.env.CI,
  });
...
if (IS_EMBED_REACT_TEST) {
  ...
  webServer.push({
    command: "yarn workspace @gsoc2/embed-react dev",
    port: 3101,
    timeout: 60_000,
    reuseExistingServer: !process.env.CI,
  });
